...
const Box = ({ data, setCommentCtrl, setRandNumForCommentClick }) => {
  return (
    <div
      ...
      className="relative flex justify-start gap-8 items-center cursor-pointer w-full h-28 p-6 rounded-lg bg-zinc-100 border-2 border-zinc-200 transition-all duration-300 hover:border-orange-500"
    >
      <div className="flex flex-col gap-4">
        <div>ایمیل: {data.email}</div>
        <div className="text-xs absolute left-3 top-3 bg-orange-500 text-white flex justify-center items-center h-6 w-26 rounded-md">
          {data.createdAt}
        </div>
        ...
        <div className="absolute bottom-3 left-32 flex justify-end items-center gap-2">
          {data.viewed == true ? (
            <div>
            </div>
          ) : (
